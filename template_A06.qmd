...
[https://github.com/patrickgao2027/Gao_Patrick_201_A06l](https://github.com/patrickgao2027/Gao_Patrick_201_A06)
# check if grader can get to your git hub
:::
...
```{r}
heightvec <- c(78,81,78,86,85)
for (i in 1: length(player_info)){
print(player_info[i])
player_info[i] <- append(player_info[[i]],heightvec[i],after=5)
}
```
...
for (i in 1:nrow(mat_rand)){
  for (j in 1:ncol(mat_rand)){
    if (mat_rand[i,j] <= 99){
      mat_rand[i,j] <- round(mat_rand[i,j],digits=0)
    }
  }
}
```
